...
class EmailTemplate {
  ...
  EmailTemplate(LoadEntryHeader invoice) {
  ...
  private List<SummaryOfProductsPrices> pricesList(LoadEntryHeader invoice) {
    List<EntryItems> products = invoice.getProducts();
    List<SummaryOfProductsPrices> pricesList = new ArrayList<>();

    for (EntryItems product : products) {
