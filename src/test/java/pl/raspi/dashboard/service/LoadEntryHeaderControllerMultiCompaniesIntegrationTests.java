...
@WithMockUser
public class LoadEntryHeaderControllerMultiCompaniesIntegrationTests {
  ...
  private LoadEntryHeader updatedInvoice;
  ...
  private long registerInvoiceBuyerAtCompanyDb(LoadEntryHeader invoice) throws Exception {
    long buyerId = addCompanyToCompanyDb(invoice.getBuyer());
    this.invoice.getBuyer().setId(buyerId);
    return buyerId;
  }

  private long registerInvoiceSellerAtCompanyDb(LoadEntryHeader invoice) throws Exception {
  ...
  public void shouldGetCorrectEntriesByDateWhenBuyerMatchesCompanyId() throws Exception {
    ...
    //then
    for (LoadEntryHeader inv : invoiceList) {
  ...
  private long addInvoiceToInvoiceDb(LoadEntryHeader invoice) throws Exception {
    String serviceResponse = this.mockMvc
