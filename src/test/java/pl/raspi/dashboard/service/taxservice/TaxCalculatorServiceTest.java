package pl.raspi.dashboard.service.taxservice;
...
import pl.raspi.dashboard.service.CompanyService;
import pl.raspi.dashboard.testhelpers.InvoicesWithSpecifiedData;
import pl.raspi.dashboard.testhelpers.TaxSummaryMapBuilder;
...
public class TaxCalculatorServiceTest {
  ...
  public void shouldCalculateIncomeWholeInvoicesInDataRange() {
    //given
    List<LoadEntryHeader> invoicesSeller = new ArrayList<>();
    for (int i = 1; i <= 12; i++) {
  ...
  public void shouldCalculateCostWholeInvoicesInDataRange() {
    //given
    List<LoadEntryHeader> invoicesBuyer = new ArrayList<>();
    for (int i = 1; i <= 6; i++) {
      LoadEntryHeader invoice = generator.getTestInvoice(i, 5);
      invoice.setIssueDate(LocalDate.now().plusMonths(i));
  ...
  public void shouldCalculateIncomeTaxAdvanceLinearTax() {
    ...
    List<LoadEntryHeader> invoices = new ArrayList<>();
    for (int i = 1; i <= 25; i++) {
      LoadEntryHeader invoice = generator.getTestInvoice(i, 1);
